...
get_latest_version() {
    ...
    RELEASE_LATEST=$(curl -s https://api.github.com/repos/FlushT/serv00-sb-vmess/releases/latest | jq -r '.tag_name')
...
download_nezha() {
    if [ -z "$VERSION" ]; then
        ...
        DOWNLOAD_LINK="https://github.com/FlushT/serv00-sb-vmess/releases/latest/download/dashboard"
        VERSION=$RELEASE_LATEST  # 将版本设置为最新版本
    else
        # 如果传入了VERSION变量，下载指定版本
        DOWNLOAD_LINK="https://github.com/FlushT/serv00-sb-vmess/releases/download/${VERSION}/dashboard"
    fi
    ...
    echo "下载版本：${VERSION}"
	curl -s https://api.github.com/repos/FlushT/serv00-sb-vmess/releases/latest | jq -r '.tag_name' > ${WORKDIR}/VERSION
